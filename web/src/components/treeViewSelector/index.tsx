...
const TreeViewSelector = ({
  ...
  const { foldList, flattenList, selectedList, handleDeptOrUserClick, handleTypeIsCanSelect, setSearchToDeptValue } = useAction({
    appId,
    defaultSelectedList,
    foldData,
    flattenData,
          ...
          return (
            <div key={deptUserData.name}>
              ...
              >
                ...
                {deptUserData.children.length > 0 && (!!deptUserData.isCollapsed ? <ExpandLess /> : <ExpandMore />)}
              </ListItemButton>
              {deptUserData.children.length > 0 && (
                <Collapse in={!!deptUserData.isCollapsed} timeout={0} unmountOnExit>
